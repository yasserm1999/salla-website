...
const steps = ["تم الاستلام", "الفحص", "التنظيف", "جاهز"];

export default function TrackerPageAr() {
  ...
  const handleSearch = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.orders) && data.orders.length > 0) {
        ...
        setOrders([]);
        setMessage(data.message || "لا توجد طلبات نشطة");
      }
    } catch (error) {
      console.error(error);
      setOrders([]);
...
const getOrderStatus = (order: any) => {
  if (!order) return "تم الاستلام";

  const status = String(order.status || "");

  // status 5 = Detailing
  if (status === "5") return "الفحص";

  // status 1 = Ready
  if (status === "1") return "جاهز";

  // status 0 = Cleaning
  if (status === "0") return "التنظيف";
...
const getStepIndex = (status: string) => {
  if (status === "الفحص") return 1;
  if (status === "التنظيف") return 2;
  if (status === "جاهز") return 3;

  return 0;
};
  const getPaymentStatus = (order: any) => {
    if (!order) return "غير مدفوع";
...
>
  ...
  <div className="relative z-10 mx-auto max-w-5xl rounded-3xl bg-white p-8 shadow-xl md:p-12">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-center text-[#546d83]">
          أدخل رقم العميل لعرض وتتبّع طلباتك.
        </p>
          ...
          <div className="mt-10">
            ...
            <div className="rounded-3xl bg-[#f5f3f0] p-6">
              <div className="mb-8 grid gap-4 md:grid-cols-3">
                <div>
                  <p className="text-sm text-[#546d83]">رقم الطلب</p>
                ...
                <div>
                  <p className="text-sm text-[#546d83]">حالة الدفع</p>
                  <p className="text-xl font-bold text-[#26364d]">
                ...
                <div>
                  <p className="text-sm text-[#546d83]">الحالة الحالية</p>
                  <p className="text-xl font-bold text-[#26364d]">
